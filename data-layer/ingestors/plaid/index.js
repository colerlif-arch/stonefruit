...
const PLAID_ACCESS_TOKEN_PATTERN = /^access-[^-]+-.+/;

console.log('[startup] node entry context', {
  __filename,
  cwd: process.cwd(),
});

const sleep = (ms) => new Promise(resolve => setTimeout(resolve, ms));
const isLikelyPlaidAccessToken = (token) =>
  typeof token === 'string' && PLAID_ACCESS_TOKEN_PATTERN.test(token);
...
// Debug endpoint: inspect transaction persistence state in SQLite
app.get('/api/debug/sqlite/transactions', function (request, response, next) {
  Promise.resolve()
    .then(function () {
      const sampleSize = Number.parseInt(request.query.sample_size, 10) || 5;
      const debugData = getTransactionsPersistenceDebug(sampleSize);
      console.log('[debug/sqlite/transactions] snapshot', {
        dbPath: debugData.dbPath,
        totalRows: debugData.totalRows,
        totalSyncStateRows: debugData.totalSyncStateRows,
      });
      response.json({ error: null, ...debugData });
    })
    .catch(next);
});

// Create custom sandbox transactions for the current Item
// https://plaid.com/docs/api/sandbox/#sandboxtransactionscreate
app.post('/api/sandbox/transactions/create', function (request, response, next) {
  Promise.resolve()
    .then(async function () {
      const latestValidItem = getLatestValidItem();
      const latestItem = getLatestItem();
      if (!latestValidItem?.access_token) {
        response.status(400).json({
          error: 'No valid items found in persistence. Link an item first so a real Plaid access_token exists in the items table.',
          latest_item_id: latestItem?.item_id || null,
        });
        return;
      }

      const accessToken = latestValidItem.access_token;
      const itemId = latestValidItem.item_id;
      // Plaid sandbox requires date_posted within the last 14 days.
      ...
      const transactions = [
        {
          ...
          date_transacted: startDate,
          description: 'Stonefruit Sandbox Transaction Start',
          iso_currency_code: 'USD',
        },
        {
          amount: 41.87,
          date_posted: endDate,
          date_transacted: endDate,
          description: 'Stonefruit Sandbox Transaction End',
          iso_currency_code: 'USD',
        },
      ];

      const sandboxCreateRequest = {
        access_token: accessToken,
        transactions,
      };

      console.log('[sandbox/transactions/create] creating transactions', {
        itemId: itemId || null,
        hasAccessToken: Boolean(accessToken),
        fixedDateRange: { startDate, endDate },
        transactionCount: transactions.length,
      });

      const sandboxCreateResponse =
        await client.sandboxTransactionsCreate(sandboxCreateRequest);

      console.log(
        '[sandbox/transactions/create] plaid response',
        sandboxCreateResponse.data,
      );

      response.json({
        error: null,
        item_id: itemId || null,
        fixed_date_range: { start_date: startDate, end_date: endDate },
        sandbox_transactions_create: sandboxCreateResponse.data,
...
app.get('/api/transactions', function (request, response, next) {
  Promise.resolve()
    .then(async function () {
      let accessTokenForSync = ACCESS_TOKEN || null;
      const latestValidItem = getLatestValidItem();
      const latestItem = getLatestItem();
      if (!accessTokenForSync && latestValidItem?.access_token) {
        accessTokenForSync = latestValidItem.access_token;
        ACCESS_TOKEN = accessTokenForSync;
      }
      if (!isLikelyPlaidAccessToken(accessTokenForSync)) {
        response.status(400).json({
          error:
            'No valid Plaid access_token is available. Call /api/set_access_token to persist a real token in items table.',
          item_id: latestValidItem?.item_id || latestItem?.item_id || ITEM_ID || null,
        });
        return;
      }
      let persistenceItemId = ITEM_ID || null;
      if (!persistenceItemId && accessTokenForSync) {
        persistenceItemId = getItemIdByAccessToken(accessTokenForSync);
        if (persistenceItemId) {
          ITEM_ID = persistenceItemId;
        }
      }
      if (!persistenceItemId && latestValidItem?.item_id) {
        persistenceItemId = latestValidItem.item_id;
        ITEM_ID = persistenceItemId;
      }
      if (!persistenceItemId && latestItem?.item_id) {
        persistenceItemId = latestItem.item_id;
        ITEM_ID = persistenceItemId;
      }
      console.log('[transactions] /api/transactions called', {
      ...
      while (hasMore) {
        ...
        console.log('[transactions] requesting transactionsSync page', {
          page,
          item_id: ITEM_ID || null,
          has_access_token: Boolean(accessTokenForSync),
          request_params: {
            cursor: cursor || null,
            access_token_set: Boolean(request.access_token),
          },
        });
        const response = await client.transactionsSync(request);
        const data = response.data;
        console.log(
          '[transactions] full transactionsSync response.data',
          util.inspect(data, { colors: true, depth: null, maxArrayLength: null }),
        );
        console.log('[transactions] received transactionsSync page', {
          page,
          item_id: ITEM_ID || null,
          has_access_token: Boolean(accessTokenForSync),
          added_size: data.added?.length ?? 0,
          modified_size: data.modified?.length ?? 0,
          removed_size: data.removed?.length ?? 0,
          has_more: data.has_more,
          next_cursor: data.next_cursor || null,
        });
        try {
          console.log('[transactions] before upsertTransactions', {
            page,
            item_id: ITEM_ID || null,
            has_access_token: Boolean(accessTokenForSync),
            persistence_item_id: persistenceItemId,
          ...
          if (!persistenceItemId) {
            console.error('[transactions] skipping upsertTransactions: no Plaid item_id available', {
              page,
              item_id: ITEM_ID || null,
              has_access_token: Boolean(accessTokenForSync),
            });
          ...
          console.log('[transactions] after upsertTransactions attempt', {
            page,
            item_id: ITEM_ID || null,
            has_access_token: Boolean(accessTokenForSync),
            persistence_item_id: persistenceItemId,
          ...
          console.error('[transactions] sqlite persistence failed', {
            page,
            message: e?.message,
            stack: e?.stack,
          });
        ...
        // Normally, we would listen for a webhook, but this app doesn't
        ...
        if (cursor === null || cursor === "") {
          warmupPolls += 1;
          console.log('[transactions] next_cursor not ready; waiting 2s before retry', {
            page,
            warmupPolls,
            maxWarmupPolls,
            nextCursor: cursor,
          ...
          if (warmupPolls >= maxWarmupPolls) {
            console.warn('[transactions] max warmup polls reached; returning current results', {
              page,
      ...
      console.log('[transactions] returning response', {
        totalAdded: added.length,
        totalModified: modified.length,
        totalRemoved: removed.length,
        latestTransactionsReturned: recently_added.length,
      });
...
const server = app.listen(APP_PORT, function () {
  console.log('stonefruit node server listening on port ' + APP_PORT);
});

const prettyPrintResponse = (value, label = '[plaid] response') => {
  const payload =
    value?.data ??
    value?.response?.data ??
    (value?.message ? { message: value.message } : value);
  console.log(
...
const formatError = (error) => {
  ...
  const responseBody = error?.response?.data || error?.data;
...
// Since this app does not support webhooks, this function can be used to poll
...
app.use('/api', function (error, request, response, next) {
  ...
  prettyPrintResponse(error, '[api] error payload');
